...
const syncProxyButton = document.getElementById("sync_proxy");
...
exportButton.addEventListener("click", async () => {
  ...
  try {
    ...
    let accName = document.getElementById("acc_name").value.trim();
    if (accName) payload.account_name = accName;
    const dlName = accName ? `${accName}.json` : "gemini-auth.json";

    await downloadJson(dlName, payload);

    const exportedNames = EXPORT_ORDER.filter((name) => info.selected.has(name));
    setStatus(
      `Created ${dlName} with ${exportedNames.length} cookie(s) and XSRF.\n\n` +
      ...
      "Move the file into gemini-web2api/cookies and do not share it or commit it to Git.",
      "ok"
    );
  } catch (error) {
    setStatus(error?.message || String(error), "warn");
...
syncProxyButton.addEventListener("click", async () => {
  syncProxyButton.disabled = true;
  setStatus("Syncing cookies to local proxy server...");
  ...
  try {
    ...
    // Send to local proxy
    const baseUrl = document.getElementById("proxy_url").value.replace(/\/+$/, "");
    
    // Save URL for next time
    try { chrome.storage.local.set({ proxyUrl: baseUrl }); } catch(e) {}
    
    const response = await fetch(`${baseUrl}/api/sync-cookies`, {
        method: "POST",
    ...
    setStatus(`Successfully synced to Proxy!\n\nProxy Response: ${data.message || 'Success'}`, "ok");

  } catch (error) {
    setStatus(`Failed to sync to proxy.\nMake sure the proxy is running and the URL is correct!\n\nError: ${error?.message || String(error)}`, "warn");
  } finally {
    syncProxyButton.disabled = false;
  }
});

// Load saved proxy URL on startup
document.addEventListener("DOMContentLoaded", () => {
    try {
        chrome.storage.local.get(["proxyUrl"], (result) => {
            if (result.proxyUrl) {
                document.getElementById("proxy_url").value = result.proxyUrl;
            }
        });
    } catch(e) {}
});
